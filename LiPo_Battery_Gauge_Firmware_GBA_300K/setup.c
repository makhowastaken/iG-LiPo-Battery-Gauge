/*
 ...
 changed ADC values for 300k resistor
...
void animate_leds(void) {
	...
	if (batteryVoltageLevel >= 710) {
		PORTD |= (1<<LED2);
		_delay_ms(DELAY_TIME);
	}
	if (batteryVoltageLevel >= 727) {
		PORTD |= (1<<LED3);
		_delay_ms(DELAY_TIME);
	}
	
	// Blue
	if (batteryVoltageLevel >= 745) {
		PORTD |= (1<<LED4);
		_delay_ms(DELAY_TIME);
	}
	if (batteryVoltageLevel >= 762) {
		PORTB |= (1<<LED5);
		_delay_ms(DELAY_TIME);
	}
	if (batteryVoltageLevel >= 780) {
		PORTD |= (1<<LED6);
		_delay_ms(DELAY_TIME);
	}
	if (batteryVoltageLevel >= 797) {
		PORTD |= (1<<LED7);
		_delay_ms(DELAY_TIME);
	}
	
	// Green
	if (batteryVoltageLevel >= 815) {
		PORTD |= (1<<LED8);
		_delay_ms(DELAY_TIME);
	}
	if (batteryVoltageLevel >= 832) {
		PORTB |= (1<<LED9);
		_delay_ms(DELAY_TIME);
	}
